<template>
  <div class="onSale">
      ...
      <el-row>
          ...
          <el-col :span="7" :offset="1">
            <el-form-item label="商品货号：" prop="art_no">
              <el-input v-model="formOnSale.form.art_no"></el-input>
            </el-form-item>
          </el-col>
          <el-col :span="6" :offset="1">
            <el-form-item label="店铺分类：" prop="shop_category_id">
              <el-cascader
                ...
                v-model="formOnSale.form.shop_category_list">
              </el-cascader>
            </el-form-item>
          </el-col>
      </el-row>
      <el-row>
        <el-col :span="6" :offset="1">
          <el-form-item label="供应链绑定：" prop="supply_bind_status">
            <el-select v-model="formOnSale.form.supply_bind_status">
          ...
          <el-form-item label="销售价格：">
            <el-col :span="11">
              <el-form-item prop="min_price">
                <el-input v-model="formOnSale.form.min_price"></el-input>
              </el-form-item>
            </el-col>
            <el-col :span="2">到</el-col>
            <el-col :span="11">
              <el-form-item prop="max_price">
                <el-input v-model="formOnSale.form.max_price"></el-input>
              </el-form-item>
            </el-col>
          </el-form-item>
        </el-col>
        <el-col :span="6" :offset="1">
          <el-form-item label="运费模板：" prop="logistics_template_id">
            <el-select v-model="formOnSale.form.logistics_template_id">
                <el-option 
                label="全部"
                value=""
              >
      ...
      <el-row>
        <el-col :span="6" :offset="1">
          <el-form-item label="条形码：" prop="bar_code">
            <el-input v-model="formOnSale.form.bar_code"></el-input>
          </el-form-item>
        </el-col>
      </el-row>
      <el-row>
    ...
    <el-row class="btn-group" v-show="formOnSale.list.length">
      <el-button size="medium" :disabled="formOnSale.selectList.length?false:true" @click="deleteBtn">删除</el-button>
      <el-button size="medium" :disabled="formOnSale.selectList.length?false:true" @click="setShelfBtn">批量下架</el-button>
      <el-button size="medium" :disabled="formOnSale.selectList.length?false:true" @click="showCategory">调整商品分类</el-button>
      <el-button size="medium" :disabled="formOnSale.selectList.length?false:true" @click="showTemplate">调整运费模板</el-button>
...
export default {
  data () {
    var validateMinPrice = (rule, value, callback) => {
      if (value) {
        if (!REG_NUMBER.test(value)) {
          ...
          this.$refs.formOnSale.validateField('max_price');
          callback();
        }
      } else {
        callback()
    ...
    var validateMaxPrice = (rule, value, callback) => {
      if (value || value === 0) {
        ...
        } else if (value < parseFloat(this.formOnSale.form.min_price)) {
  ...
  methods: {
    // 导出报表
    exportGoodTable() {
      goods.exportGoods(this.formOnSale.form).then((res)=>{
        res.data.map((item) => { 
    ...
    getCategoryId(value) {
      this.formOnSale.form.shop_category_id = value[value.length-1]
    },
    ...
    handleSizeChange (val) {
      ...
      this.getItemList(this.formOnSale.form)
    },
    ...
    resetForm () {
      ...
      this.formOnSale.form.shop_category_list = []
    },
    deleteBtn () {
      let params = {
        item_ids: this.formOnSale.selectList
      }
    ...
    setShelfBtn (status) {
      ...
      this.$confirm(`此操作将下架已选择的全部商品, 是否确定?`, '提示', {
        closeOnClickModal: false,
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
    ...
    getItemList (params) {
      goods.getItemList(params).then((res) => {
        ...
        this.formOnSale.loading = false
        this.formOnSale.total = res.data.total_count
        ...
        this.formOnSale.list.sort(function (a, b) {
  ...
  mounted () {
    if(this.$route.query){
      this.formOnSale.form.shop_category_id = this.$route.query.category_id
    }
    this.search()
  }
}
