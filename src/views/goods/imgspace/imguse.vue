      ...
      <el-container>
        ...
        <el-main class="imgspace_imgcontent">
            <div class="imgspace_right_head">
              ...
              <!-- <el-button size="small" @click="moveFile" v-show="imgSelectList.length + fileSelectList.length > 0">移动到</el-button> -->
              <!-- <el-button size="small" @click="deleFile" v-show="imgSelectList.length + fileSelectList.length > 0">删除</el-button> -->
            ...
            <div class="imgspace_content">
              ...
              <div v-if="imglist.length > 0" class="imgspace_content_imgui">
                <div  v-for="(item,index) in imglist" :key="index" >
                  <div v-if="item.content_type === 'image'" :class="selcctImgListUrl.indexOf(item.url) != -1  ? 'imgspace_imgcontent_li_select' : 'imgspace_imgcontent_li'" @click="selectItem(item)">
...
export default {
  ...
  methods: {
    ...
    // },
    // deleFile(){
    //   if (this.imgSelectList.length > 0 || this.fileSelectList.length > 0) {
    //     let _deldata = {
    //       file_ids : this.fileSelectList,
    //       image_ids : this.imgSelectList
    //     }
    ...
    selectItem(item){
      ...
      let _store = JSON.parse(JSON.stringify(this.selectImgList))
      if (item.select) {
        if (item.content_type === "image") {
          if (this.selcctImgListUrl.length > 0) {
              if (this.selcctImgListUrl.indexOf(item.url) == -1) {
                // 数量处理
                if (_store.length > 9) {
                  this.$message({
                    message: '已达到最大上传数量',
                  ...
                  _store.push(item)
                  this.imgSelectList.push(item.id)
                }
              }
          }else{
            this.imgSelectList.push(item.id)
        ...
        if (item.content_type === "image") {
          if (this.selcctImgListUrl.length > 0) {
              let _index = this.selcctImgListUrl.indexOf(item.url)
              if (_index != -1) {
                _store.splice(_index , 1)
                this.imgSelectList.splice(_index , 1)
              }
          }
        }
      }
      this.setImgSelect(_store)
    },
    searchImg(){
  ...
  computed : {
    ...mapGetters(['selectImgList' , 'selcctImgListUrl'])
  },
  mounted () {
    goods.getImgDir({}).then((res) => {
        let _data = res.data
