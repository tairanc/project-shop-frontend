            ...
            <div class="imgspace_content">
              ...
              <div v-if="imglist.length > 0" class="imgspace_content_imgui">
                <div  v-for="(item,index) in imglist" :key="index" >
                  <div v-if="item.content_type === 'image'" :class="brandImgUrlList.indexOf(item.url) != -1  ? 'imgspace_imgcontent_li_select' : 'imgspace_imgcontent_li'" @click="selectItem(item)">
...
export default {
  ...
  methods: {
    ...
    selectItem(item){
      ...
      let _store = JSON.parse(JSON.stringify(this.selectBrandImgList))
      if (item.select) {
        if (item.content_type === "image") {
          if (this.brandImgUrlList.length > 0) {
              if (this.brandImgUrlList.indexOf(item.url) == -1) {
                // 数量处理
                if (_store.length > 9) {
                  this.$message({
                    message: '已达到最大上传数量',
        ...
        if (item.content_type === "image") {
          if (this.brandImgUrlList.length > 0) {
              let _index = this.brandImgUrlList.indexOf(item.url)
              if (_index != -1) {
  ...
  computed : {
    ...mapGetters(['selectBrandImgList' , 'brandImgUrlList'])
  },
  mounted () {
    goods.getImgDir({}).then((res) => {
        let _data = res.data
