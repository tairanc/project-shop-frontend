<template>
  <div class="detail">
    ...
    <el-row>
      ...
      <el-col>
        <p>活动规则： 消费满{{message.rules.exchange_full}}元，可换购{{message.rules.exchange_count}}件</p>
      </el-col>
      <el-col>
    ...
    <div class="table">
      <el-table :data="goodsListGift" border >
        ...
        <el-table-column align="center" label="换购价" prop="price">
        </el-table-column>
        <el-table-column align="center" label="操作">
          <template slot-scope="scope">
    ...
    methods: {
      ...
      getExchangeBuyDetail(params){
        exchange.getExchangeBuyDetail(params).then((res)=>{
          if(res.code == 0){
            this.message = res.data.activity
      ...
      handleDelete(row){
        exchange.closeItem({
          ...
          if(res.code == 0){
            ...
            this.getExchangeBuyDetail(this.pageInfo)
          }
        })
      },
      ...
      exportForm() {
        ...
        if (_this.pageInfo.item_search != null && _this.pageInfo.item_search.length > 0) {
          ...
          exchange.getExportData({promotion_id: this.pageInfo.promotion_id}).then((res)=>{
