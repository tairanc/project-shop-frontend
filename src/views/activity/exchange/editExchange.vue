  ...
  <el-form :model="addCoupon" :rules="addRules" ref="addCoupon" label-width="120px">
    <div class="basic">
      ...
      <el-form-item label="活动规则:" prop="rules">
        消费满<el-input :disabled="isduring"  v-model="addCoupon.rules.exchange_full"></el-input>元，可换购<el-input :disabled="isduring" v-model="addCoupon.rules.exchange_count"></el-input>件
      </el-form-item>
      <el-form-item label="活动开始时间:" prop="start_time">
        <el-date-picker :disabled="isduring" v-model="addCoupon.start_time" type="datetime" format="yyyy-MM-dd HH:mm:ss" value-format="yyyy-MM-dd HH:mm:ss"
          placeholder="活动开始时间">
        </el-date-picker>
      </el-form-item>
      <el-form-item label="活动结束时间:" prop="end_time">
        <el-date-picker :disabled="isduring" v-model="addCoupon.end_time" type="datetime" format="yyyy-MM-dd HH:mm:ss" value-format="yyyy-MM-dd HH:mm:ss"
          placeholder="活动结束时间">
        </el-date-picker>
      </el-form-item>
    </div>
    ...
    <div class="gifts">
      ...
      <el-row class="pl20">
        ...
        <el-table class="table" :data="goodsListGift" border align="center">
          ...
          <el-table-column align="center" label="换购价（元）" >
            <template slot-scope="scope">
    ...
    <select-goods 
                  ...
                  @choose-spec="handgiftspec"></select-goods>
    <select-goods v-else
                  :promotionType="type" 
                  :promotionId="(addCoupon.promotion_id).toString()" 
                  :selectList="selectListGift" 
                  :selectVisible="selectVisibleGift" 
                  @visible-change="handleVisibleGift" 
                  @choose-submit="handleAddGift"></select-goods>
...
<script>
  ...
  export default {
    ...
    methods: {
      //获取初始数据
      getExchangeBuyDetail(params){
        exchange.getExchangeBuyDetail(params).then((res)=>{
          if(res.code == 0){
            this.addCoupon.promotion_name = res.data.activity.name
            this.addCoupon.rules = res.data.activity.rules
            this.addCoupon.start_time = res.data.activity.start_time
            this.addCoupon.end_time = res.data.activity.end_time
            this.showBtnBatch = res.data.activity.status == 10 ? false : true

            this.goodsList = res.data.items.activity_items
            this.goodsListGift = res.data.items.exchange_items
            ...
            let _deletspec = JSON.parse(JSON.stringify(res.data.items.activity_items))
            let _data = []
            ...
            // this.goodsinduring = JSON.parse(JSON.stringify(res.data.items.activity_items))
            ...
            let _deletspecgift = JSON.parse(JSON.stringify(res.data.items.exchange_items))
            let _datagift = []
            ...
            // this.goodsgiftinduring = JSON.parse(JSON.stringify(res.data.items.exchange_items))
          }
        })
      },
      // ID添加商品弹框
      ...
      addGoodsById(type) {
        if (this.goodsIds || this.goodsIdsGift) {
          let promotion_type_code = type=='goods'?'ExchangeBuy':'exchange'
          let _item_id = ''
        if (type == 'goods') {
      ...
      //验证赠送数量
      handleCheck(row){
        const posInteger = /^(\d+)$|^(\d+\.\d+)$/

        if(!posInteger.test(row.price) || parseFloat(row.price)>row.sell_price || row.price == 0){
            this.$message({
              showClose: true,
              message: '仅限输入正数，换购价不得大于标准售价',
              type: 'warning'
            })
          }else{
            row.edit = false
          ...
          if (valid) {
            ...
            if (this.isduring) {
              ...
              let _postData = JSON.parse(JSON.stringify(this.addCoupon))
                  ...
                  _postData.exchanges = _listgift
                  if (_list.length < 1 && _listgift.length < 1) {
                    this.appendOk();
                    return
                  }
                  exchange.appendItems(_postData).then((res)=>{
                  if(res.code==0){
              ...
              this.addCoupon.arguments.exchanges = argumentsList
              this.addCoupon.arguments = JSON.stringify(this.addCoupon.arguments)

              exchange.updateExchangeBuy(this.addCoupon).then((res)=>{
                if(res.code==0){
                  this.$message({
                    showClose: true,
                    message: '创建成功',
