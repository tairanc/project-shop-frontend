<template>
  <el-form :model="addCoupon" :rules="addRules" ref="addCoupon" label-width="120px">
    <div class="basic">
      ...
      <el-form-item label="活动开始时间:" prop="end_time">
...
<script>
  ...
  export default {
    ...
    methods: {
      //获取详情
      getDirectReductionDetail(params){
        direct.getDirectReductionDetail(params).then((res)=>{
          if(res.code == 0){
            ...
            let _deletspec = JSON.parse(JSON.stringify(res.data.itemList.list))
            let _data = []
            ...
            // this.goodsinduring = JSON.parse(JSON.stringify(res.data.itemList.list))
          }
        })
      },
      // ID添加商品弹框
      ...
      addGoodsById() {;
        if (this.goodsIds) {
          ...
          ticket.importItemsById({
            item_id: _item_id,
            promotion_type: 'DirectReduction',
      ...
      handleCheck(row){
        ...
        this.goodsList.map((item)=>{
          if(item.item_id == row.item_id){
            item.promotion_price = item.price.length < 1 ? "" : parseFloat(item.price)

            if(!posNumber.test(item.price) || item.promotion_price > parseFloat(item.sell_price)||item.promotion_price==0){    
              this.$message({
                showClose: true,
                dangerouslyUseHTMLString: true,
        ...
        //关闭直降商品
      handleClose(row){
        direct.closeDirectReductionItem({
      ...
      handleSubmit(){
        this.$refs.addCoupon.validate((valid) => {
          if (valid) {
            ...
            direct.updateDirectReduction(this.addCoupon,this.isduring).then((res)=>{
                if(res.code==0){
    ...
    created() {
      ...
      this.addCoupon.promotion_id = this.$route.query.id
      this.getDirectReductionDetail({
        promotion_id: this.addCoupon.promotion_id,
        page: 1,
        page_size: -1
