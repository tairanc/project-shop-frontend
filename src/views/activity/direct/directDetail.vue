      ...
      <div class="table">
        <el-table :data="goodsList" border :span-method="spanMethod" >
          ...
          <el-table-column align="center" label="操作">
            <template slot-scope="scope">
              <el-button type="text" :disabled="(scope.row.status==0)||(message.status==10)||(message.status==40)||(message.status==50)" @click="handleDelete(scope.row)">{{scope.row.status==0?'已关闭':'关闭'}}</el-button>
            </template>
          </el-table-column>
        </el-table>
      </div>
    ...
    methods: {
      ...
      handleDelete(row){
        direct.closeDirectReductionItem({
          ...
          if(res.code == 0){
            ...
            this.getDirectReductionDetail(this.pageInfo)
          }
        })
      },
      // pageSize变化函数
      ...
      exportForm() {
        ...
        if (_this.pageInfo.item_search != null && _this.pageInfo.item_search.length > 0) {
          ...
          direct.getDirectReductionDetail(this.pageInfoAll).then((res)=>{
