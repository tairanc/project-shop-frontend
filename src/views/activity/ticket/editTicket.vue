<template>
  <el-form :model="addCoupon" :rules="addRules" ref="addCoupon" label-width="120px">
    ...
    <div class="all">
      <el-row>
        <el-col :span="16">
          ...
          <div class="pl20 pt20">
            <table-view ref="goodsTable" :loading="loading" :columns="columns" :pageList="itemList"></table-view>          
          </div>
        </el-col>
      </el-row>
    </div>
    ...
    <el-row>
      ...
      <el-row>
        <el-col :span="15">
          <el-form-item label="使用开始时间:" prop="use_start_time"  v-if='addCoupon.is_set_expiry_date == 0'>
      ...
      <el-row>
        <el-col :span="15">
          <el-form-item label="使用结束时间:" prop="use_end_time"  v-if='addCoupon.is_set_expiry_date == 0'>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      addGoodsById() {;
        if (this.goodsIds) {
          ...
          if (this.showBtnBatch == false) {
            ...
            this.itemList.map((mitem, mindex)=>{
      ...
      goodsFilter(items) {
        for (let i = 0; i < items.length; i++) {
          ...
          if (this.itemList.length != 0) {
            for (let j = 0; j < this.itemList.length; j++) {
              if (items[i].id == this.itemList[j].id) {
                flag = true;
              }
            }
          }
        ...
        for(let n = 0; n < this.itemList.length;n++){
          if(list.indexOf(this.itemList[n].id) == -1){
      ...
      getEditcoupon(id) {
        let that = this;
        ticket.getShopcoupon({
          coupon_id: id
        }).then((res) => {
          const detail = res.data;
          Object.keys(that.addCoupon).forEach(function (key) {
            that.addCoupon[key] = detail[key] || detail.extend[key];
          });
          that.addCoupon.coupon_name = detail.name;
          that.addCoupon.is_show = detail.is_show.toString();
          that.addCoupon.is_set_expiry_date =detail.is_set_expiry_date +''
          that.addCoupon.is_check_all = parseInt(JSON.parse(detail.extend.arguments).is_check_all)
          that.addCoupon.is_set_expiry_date = parseInt(JSON.parse(detail.extend.arguments).is_set_expiry_date) +''
          that.addCoupon.applicable_platform = this.addCoupon.applicable_platform.split(",")
          ...
          this.showBtnBatch = res.data.status == 1 ? false : true
        });
      },
      // 获取优惠券报名的商品
        ...
        if (this.addCoupon.item_ids == "") {
          ...
          this.$refs.addCoupon.validate((valid) => {
            if (valid) {
              ...
              ticket.updateCounpon(that.commitData).then((res) => {
                if (res.code == 0) {
                  this.$message({
                    showClose: true,
                    dangerouslyUseHTMLString: true,
    ...
    created() {
      ...
      this.getEditcoupon(this.pageInfo.coupon_id);
