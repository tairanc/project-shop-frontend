<template>
  <el-form :model="addCoupon" :rules="addRules" ref="addCoupon" label-width="120px">
    ...
    <select-goods :promotionType="type" :promotionId="(addCoupon.promotion_id).toString()" :selectList="selectList" :selectVisible="selectVisible" @visible-change="handleVisible" @choose-submit="handleAdd"></select-goods>
      ...
      getOptionBuyDetail(params){
        option.getOptionBuyDetail(params).then((res)=>{
          if(res.code == 0){
            ...
            this.addCoupon.amount = res.data.rules.amount
            this.addCoupon.quantity = res.data.rules.quantity
      ...
      handleSubmit(){
        this.$refs.addCoupon.validate((valid) => {
          if (valid) {
            ...
            option.updateOptionBuy(this.addCoupon).then((res)=>{
              if(res.code==0){
                this.$message({
                  showClose: true,
                  message: '创建成功',
