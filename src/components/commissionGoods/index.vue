        ...
        <div class='right'>
            ...
            <ul>
                <li v-for="goods in goodList" :key="goods.id">
                    ...
                    <el-input v-model="goods.rate" style="dispaly:inline-block;width:80px;vertical-align: top;"></el-input><span style="vertical-align: top">%</span>
                    <div class='title'>
                      ({{goods.title}})
                    </div>
    ...
    handleSubmit(){
      if(this.selectItems.length){
        ...
        for(var i in this.goodList){     
            var data ={item_id:'',effecting_rate:'',category_id:''} 
            data.item_id =this.goodList[i].id
            data.effecting_rate =this.goodList[i].rate
            data.category_id =this.goodList[i].category_id
            items.push(data)
        }
        this.createCommissionItem({items:items})
